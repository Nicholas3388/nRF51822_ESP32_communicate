...
static void nus_data_handler(ble_nus_t * p_nus, uint8_t * p_data, uint16_t length)
{
    if(((p_data[0]=='O')||(p_data[0]=='o'))&&((p_data[1]=='N')||(p_data[1]=='n')))  //如果收到的数据为“ON”或者"on"
		{
		 nrf_gpio_pin_clear(LED_2 );    //工作指示绿灯亮
		 nrf_gpio_pin_set(LED_3);        //工作指示红灯灭
		 states[0]=p_data[0];               //更新要上传的状态数据
		 states[1]=p_data[1];
		 states[2]=0;
		 change_state=1;                  //状态发生变化，马上上传状态一次
		}
		
		 if(((p_data[0]=='O')||(p_data[0]=='o'))&&((p_data[1]=='F')||(p_data[1]=='f'))&&((p_data[2]=='F')||(p_data[2]=='f')))
		{
		 //如果收到的数据为“OFF”或者"off"
		 nrf_gpio_pin_clear(LED_3);     //工作指示红灯亮
		 nrf_gpio_pin_set(LED_2);    //工作指示绿灯灭
		 states[0]=p_data[0];             //更新要上传的状态数据
		 states[1]=p_data[1];
		 states[2]=p_data[2];
		 change_state=1;                  //状态发生变化，马上上传状态一次
